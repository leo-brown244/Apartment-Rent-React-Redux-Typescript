  ...
  return (
    ...
    >
      ...
      <DialogContent className={classes.container}>
        {error && (
          <Grid item md={true} sm={true} xs={true} className={classes.errorContainer}>
            <p className={classes.errorFont}>{error}</p>
          </Grid>
        )}
        ...
        <Grid item md={true} sm={true} xs={true} className={classes.rowContainer}>
          <TextField
            ...
            onBlur={(event)=>{
              ...
              if(!value)
                return;
              let geocodeAddress = `https://maps.googleapis.com/maps/api/geocode/json?address=${value}&key=AIzaSyDHaXYynYE6xzzbzmA7Rg5h1-S9BZ_uthc`;
              fetch(geocodeAddress).then(res=>res.json()).then(result=>{
                if(result.results.length == 0)
                  return;
                let latitude:number = result.results[0].geometry.location.lat;
                let longitude:number = result.results[0].geometry.location.lng;
        ...
        <Grid container spacing={3} className={classes.rowContainer}>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              //   const latitude = form[ApartmentFormKeys.coordinates] ? form[ApartmentFormKeys.coordinates]![0] : 0;
              //   const longitude = form[ApartmentFormKeys.coordinates] ? form[ApartmentFormKeys.coordinates]![1] : 0;
              //   let geocodAddress = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=AIzaSyDHaXYynYE6xzzbzmA7Rg5h1-S9BZ_uthc`;
              //   fetch(geocodAddress).then(res=>res.json()).then(result=>{
              //     console.log(result)
              //   })
              //   }
              // }
