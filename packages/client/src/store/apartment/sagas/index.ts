...
import { getApartmentListFailure, getApartmentListSuccess, setApartmentFormFieldError, submitApartmentFormSuccess, submitApartmentFormFailure, clearApartmentFormField, updateApartmentSuccess, updateApartmentFailure, removeApartmentSuccess, removeApartmentFailure, setDialogOpenStatus, setEditDialogOpenStatus } from '../actions';
...
function* validateApartmentForm() {
  ...
  else if (form[ApartmentFormKeys.size] <= 0) {
    hasError = true;
    yield put(setApartmentFormFieldError({
      key: ApartmentFormKeys.size,
      value: 'Floor Area Size: Needs to be positive'
    }));
  }
  ...
  else if (form[ApartmentFormKeys.price]<=0) {
    hasError = true;
    yield put(setApartmentFormFieldError({
      key: ApartmentFormKeys.price,
      value: 'Price per month: Needs to be positive'
    }));
  }
  ...
  else if (form[ApartmentFormKeys.roomNums]<=0) {
    hasError = true;
    yield put(setApartmentFormFieldError({
      key: ApartmentFormKeys.roomNums,
      value: 'Number of Rooms: Needs to be positive'
    }));
  }
...
function* updateApartmentForm() {
  try {
    ...
    if (!hasError) {
      ...
      yield put(clearApartmentFormField());
      yield put(setEditDialogOpenStatus(false));
    }
  } catch (error) {
